...
class validar extends conectar
	...
	public function consultaadmintoken($id, $conexion)
		{
		$token=$this->token();
		$sql="SELECT id, validacion  FROM sa WHERE id='".mysql_real_escape_string($id)."' AND validacion='".mysql_real_escape_string($token)."'";
...
public function cerrarSesion($parametro)
{
	...
	$sql="UPDATE sa SET  validacion='cerrado' WHERE id='".intval($id)."'";
	if(mysql_query($sql))
	{
