...
public function datosBasicosItem($parametros)
{
	...
	nombreArticulo='".$this->filtroStrings($nombreArticulo, 2)."',
	ventaFinal='".$this->filtroStrings($this->normalizacionDeCaracteres($ventaFinal), 0)."',
	ventaProvedor='".$this->filtroStrings($this->normalizacionDeCaracteres($ventaProvedor), 0)."'
	WHERE id='".intval($this->decrypt($idItem, publickey))."'
	";
	if (mysql_query($sql)) {
		# code...
		return 1;
