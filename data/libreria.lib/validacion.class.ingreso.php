...
class validar extends conectar
	{
	public function administrador($datos)
		{
		conectar::conexiones();
		extract($datos);
		...
		$rs=mysql_fetch_array($this->consultaadmin($u_ser,$p_sword,$this->conexion));
		...
		if(isset($rs[0]))
			...
			if($this->ingresotoken($token,$rs[0],$this->conexion)=="ok")
				...
				{
				$done="administrador/index.php?id=".$this->encrypt($rs[0], publickey);
		...
		else
			{
				...
				//Chequeo la  existencia  del usuario  vendedor
				$rs=mysql_fetch_array($this->consultaVendedor($u_ser,$p_sword,$this->conexion));
				
				$token=$_SESSION["autenticar"]=$this->token(rand(23,88),TRUE,TRUE,FALSE);
				if($this->ingresotokenVendedor($token,$rs[0],$this->conexion)=="ok")
				{
				$done="ventas/index.php?id=".$this->encrypt($rs[0], publickey);
				header('Location: '.$done.'');
				}
	...
	public function consultaadmin($u_ser,$p_sword, $conexion)
		{

		  $sql="SELECT id, usuario, contrasena FROM sa WHERE usuario='".$this->encrypt($u_ser, key)."' AND contrasena='".$this->encrypt($p_sword,key)."'";
		...
		//Selecciona y  filtra  los usuarios a partir de  su usuario y contrasena
	public function consultaVendedor($u_ser,$p_sword, $conexion)
		{

		  $sql="SELECT id, email, contrasena FROM empleados WHERE email='".$u_ser."' AND contrasena='".$this->encrypt($p_sword,key)."'";
		  return mysql_query($sql,$conexion);
		}
	...
	private function ingresotoken($token,$id, $conexion)
		{
		$sql="UPDATE sa SET validacion='".$token."', ipReal='".$this->getRealIP()."'  WHERE id='".mysql_real_escape_string($id)."'";
	...
	//ingreso la llave al administrador que acaba de iniciar sesion
	private function ingresotokenVendedor($token,$id, $conexion)
		{
		$sql="UPDATE empleados SET validacion='".$token."', ipReal='".$this->getRealIP()."'  WHERE id='".mysql_real_escape_string($id)."'";
		if(mysql_query($sql,$conexion)){return "ok";} else { echo "no";};
		}
